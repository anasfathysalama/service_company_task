...
class SenderParcelsDatatable extends DataTables
{
    ...
    {
        $query = $this->model::query()->where('sender_id' , auth()->user()->id)->select(DB::raw('count(*) as rows_count'));
    ...
    {
        $query = $this->model::where('sender_id' , auth()->user()->id)->orderBy($this->getOrderColumnName(), $this->getOrderType())
